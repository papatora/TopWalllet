...
export function evidenceBlocks(i) {
  ...
  const out = w[2].map((li, j) => {
    ...
    else if (L === 'INSIDER') body = row('Tokens', (d.tokens || []).map(tok).join(', ')) + row('Sells without a buy', d.sell_count) + (d.sample_sells || []).slice(0, 4).map(s => row(`block ${nf(s.block)}`, tx(s.tx_hash))).join('');
